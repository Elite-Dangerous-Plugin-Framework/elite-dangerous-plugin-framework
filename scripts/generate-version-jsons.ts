...
const data = ReleasesSchema.parse(await fetch("https://api.github.com/repos/Elite-Dangerous-Plugin-Framework/elite-dangerous-plugin-framework/releases").then(e => e.json()))

async function withLatestJson(item: z.infer<typeof ReleasesSchema>[number] | undefined) {
  if (!item) {
    return undefined
  }
  const searchUrl = item.assets.find(e => e.name === "latest.json")?.browser_download_url;
  if (!searchUrl) {
    return undefined
  }
...
const mostRecentStableRelease = await withLatestJson(data.filter(e => !e.prerelease).toSorted((a, b) => b.tag_name.localeCompare(a.tag_name)).find(() => true))
const mostRecentPrerelease = await withLatestJson(data.filter(e => e.prerelease).toSorted((a, b) => b.tag_name.localeCompare(a.tag_name)).find(() => true))

const sharedMostRecent = [mostRecentPrerelease, mostRecentStableRelease].filter(Boolean).toSorted((a, b) => b!.tag_name.localeCompare(a!.tag_name)).find(() => true)
if (!sharedMostRecent)
  throw new Error("No releases at all!")
