...
export class GameState {
  ...
  private handleEvent(ev: JournalEvent_BI, allowEmitToEDDN: boolean) {
    ...
    // at this point the "internal" state is set. We now handle EDDN (if allowed)
    if (allowEmitToEDDN && this.#cmdr && this.#header && this.#state?.system) {
      const message = extractAndStripJournal(ev, this.#state.system.starPos);
      const header = makeEddnHeader("0.0.1", this.#cmdr, this.#header);
      if (message && header) {
        const entirePayload = {
  ...
  public static fromInitialState(
    ...
    const state = new GameState(file, eventCallback);
